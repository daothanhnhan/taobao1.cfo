           ...
           if($file_size < 50 || true) {
               ...
               if($isUploaded) {
                    ...
                    for($row =2; $row <= $total_rows; $row++) {
                        ...
                        var_dump($single_row);
                        $InvDate = date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($single_row[0][2]));var_dump($InvDate);
                        die;
                        // echo "<tr>";
...
$cart_id = 0;
                    for($row =2; $row <= $total_rows; $row++) {
                        $query = "insert into `product` (productcat_ar, xay_dung_id, product_code, `product_name`, product_img_1, product_price, `product_price_sale`, `product_shape`, product_stock, `product_sub_info3`, `product_sub_info2`, `product_content`, product_sub_info1, friendly_url, title_seo) VALUES ";
                        //Read a single row of data and store it as a array.
                        //This line of code selects range of the cells like A1:D1
                        ...
                        foreach($single_row[0] as $key=>$value) {
                            ...
                            if ($d <=13) {
                                echo "<td>".$value."</td>";
                                $query .= "'".mysqli_real_escape_string($conn_vn, $value)."',";
                                if ($d==3 || $d==4 || $d==10 || $d==11 || $d==12 || $d==13) {
