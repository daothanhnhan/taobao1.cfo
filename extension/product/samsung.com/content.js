...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if(request.method == "changePage"){
            ...
            var price = document.getElementsByClassName("cost-box__price-original")[0];

            if (price) {
            	var price1 = document.getElementsByClassName("cost-box__price-original")[0];
	            ...
	            // alert(price2);
	            var price2 = document.getElementsByClassName("cost-box__price-now")[0];
	            price2 = price2.innerText;
	            price2 = price2.replace(/\D/gm,"");
	            // alert(price2);
            } else {
            	// alert('1');
            	// var price_1 = document.getElementsByClassName("product-detail-info__price")[0];
            ...
            var feature_1 = document.getElementsByClassName("feature-benefit")[0];
            if (feature_1) {
            	var content_arr = document.getElementsByClassName("feature-benefit");
            }

            var feature_2 = document.getElementsByClassName("feature-benefit-full-bleed")[0];
            if (feature_2) {
            	var content_arr = document.getElementsByClassName("feature-benefit-full-bleed");
            }
            
            var feature_3 = document.getElementsByClassName("mgFeature")[0];
            if (feature_3) {
            	var content_arr = document.getElementsByClassName("mgFeature");
            }


            var content_count = content_arr.length;
            // alert(content.length);
            for (var i=0;i<content_count;i++) {
            	content += content_arr[i].innerHTML;
            	// alert(content);
            }
            ...
            var img = document.getElementsByClassName("pd-header-gallery__item swiper-slide");
			...
			for (i=0;i<img_pro_arr.length;i++) {
				...
				link_src = link_src.replace('$LazyLoad_Home_PNG$', '');
				link_src = link_src.replace('$650_519_PNG$', '');
				// alert(src[0]);
            ...
            if (img_content != null) {
            	for (i=0;i<img_content.length;i++) {
					...
					link_src = link_src.replace('//', 'https://');
					link_src = link_src.replace('$LazyLoad_Home_PNG$', '');
					link_src = link_src.replace('$ORIGIN_PNG$', '');
