...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if(request.method == "changePage"){
            ...
            var price = document.getElementsByClassName("product-details_heading_pricing-before-discount")[0];

            if (price) {
            	var price1 = document.getElementsByClassName("product-details_heading_pricing-before-discount")[0];
	            ...
	            // alert(price2);
	            var price2 = document.getElementsByClassName("product-details_heading_pricing-number")[0];
	            price2 = price2.innerText;
	            price2 = price2.replace(/\D/gm,"");
	            // alert(price2);
            } else {
            	// alert('1');
            	var price1 = document.getElementsByClassName("product-details_heading_pricing-number")[0];//alert(price1);
            ...
            des = '';
            var content = document.getElementsByClassName("card-content")[0];
            ...
            for (var i=0;i<img_pro_length;i++) {
            	...
            	imgTags = img[i].src;
            	imgTags = imgTags.replace('small', 'medium');
            ...
            var img_ul = document.getElementsByClassName("slick-dots")[0];
            if (img_ul) {
            	img_pro_arr = [];
            	var li = img_ul.getElementsByTagName("li");
	            // alert(li.length);
	            img_pro_length = li.length;

	            for (var i=0;i<img_pro_length;i++) {
	            	// alert(li[i].innerHTML);
	            	imgTags = li[i].innerHTML.match(/<img [^>]*src="[^"]*"[^>]*>/gm);
	            	// imgTags = img[i].src;
	            	img_pro_arr.push(imgTags);
	            	// break;
	            }
			...
			for (i=0;i<img_pro_arr.length;i++) {
				...
				link_src = link_src.replace('/globalassets', 'https://www.electrolux.vn/globalassets');
				link_src = link_src.replace('w=40', 'preset=medium');
				// alert('3');
				// alert(src[0]);
			...
			sendResponse({text: h2.innerText, price1: price1, price2: price2, des: des, content: content, img: img_pro_arr, img_content: arr_img_content, method: "changePage"});
        }
