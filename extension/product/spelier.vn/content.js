...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if(request.method == "changePage"){
            ...
            var img = document.getElementsByClassName("zoomWindowContainer");
            ...
            for (var i=0;i<img_pro_length;i++) {
            	// alert(img[i].innerHTML);
            	// imgTags = img[i].innerHTML.match(/<img [^>]*src="[^"]*"[^>]*>/gm);
            	imgTags = img[i].innerHTML.match(/url\(&quot;[^"]*&quot;/gm);
            	// alert(imgTags);
            	// alert(typeof imgTags[0]);

            	imgTags = imgTags[0].replace("url(&quot;", "");
            	imgTags = imgTags.replace("&quot;", "");
            	// imgTags = img[i].src;
            ...
            var img_list = document.getElementsByClassName("img-responsive")[0];
            if (img_list) {
	            ...
	            for (var i=0;i<img_pro_length;i++) {
	            	imgTags = img[i].src;
	            	imgTags = imgTags.replace("small", "large");
	            	img_pro_arr.push(imgTags);
	            }
	            // alert(img_pro_arr);
            }
