...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if(request.method == "changePage"){
            ...
            var price = document.getElementsByClassName("box-price-old")[0];

            if (price) {
            	var price1 = document.getElementsByClassName("box-price-old")[0];
	            ...
	            // alert(price2);
	            var price2 = document.getElementsByClassName("box-price-present")[0];
	            price2 = price2.innerText;
	            price2 = price2.replace(/\D/gm,"");
	            // alert(price2);
            } else {
            	// alert('1');
            	var price1 = document.getElementsByClassName("box-price-present")[0];//alert(price1);
            ...
            var des = document.getElementsByClassName("parameter-item")[0];//alert(des.innerHTML);
			...
			var str = des;
			var subst = '$2';
			var des = str.replaceAll(re, subst);
            // alert(des);

            var img_spec = document.getElementsByClassName("img")[0];
            var img_spec_img = img_spec.innerHTML.match(/<img [^>]*src="[^"]*"[^>]*>/gm);
            var img_spec_img_src = img_spec_img[0].match(/src="[^"]*"/gm);
            img_spec_img_src = img_spec_img_src[0].substring(5);
            var img_spec_img_src_length = img_spec_img_src.length;//alert(img_spec_img_src_length);
            img_spec_img_src = img_spec_img_src.substring(0, img_spec_img_src_length-1);
            img_spec_img_src = img_spec_img_src.replace("//", "https://");
            // alert(img_spec_img_src);
            // link_src = link_src.substring(0, length_scr-1);
            

            var content = document.getElementsByClassName("content-article")[0];
            // var content1 = document.getElementsByClassName("img-main")[0];
            // var content2 = document.getElementsByClassName("content-t-wrap")[0];
            ...
            // alert('1');
            var img = document.getElementsByClassName("owl-item");
			...
			for (i=0;i<img_pro_arr.length;i++) {
				...
				link_src = link_src.substring(0, length_scr-1);
				dau = link_src.substr(0, 2);
				if (dau == '//') {
					link_src = link_src.replace("//", "https://");
				}
